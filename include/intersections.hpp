...
public:
    ...
    std::vector<Point2D> operator()(const auto& obj1, const auto& obj2) {
        throw std::logic_error("Unsupported intersection");
    }
};

inline std::optional<std::vector<Point2D>> GetIntersectionPoints(const Shape &shape1, const Shape &shape2) {
    try {
