...
GeometryResult<std::vector<Point2D>> IntersectionVisitor::operator()(const LineSegment &segment, const Circle &circle) const {
...
GeometryResult<std::vector<Point2D>> IntersectionVisitor::operator()(const Circle &circle1, const Circle &circle2) const {
    ...
    if (std::abs(y) < 1e-10) {
        return std::vector{middle};
    }
    Point2D norm = ((circle2.center - circle1.center) / d).NormalVector();
    return std::vector{middle - norm * y, middle + norm * y};
}
