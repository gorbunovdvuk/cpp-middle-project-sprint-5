...
void PrintAllIntersections(const Shape &shape, std::span<const Shape> others) {
    ...
    std::ranges::for_each(
        others | std::views::filter([&shape](const auto &other) { return Distinct(shape, other); }) |
            std::views::transform([&shape](const auto &other) {
...
void PerformShapeAnalysis(std::span<const Shape> shapes) {
    ...
    std::visit(
        [](const auto &shape1, const auto &shape2) {
            std::println("Distance between {} and {} is {}", shape1, shape2,
                         queries::DistanceBetweenShapes(shape1, shape2).value());
        },
        shape1, shape2);
}
...
void PerformExtraShapeAnalysis(std::span<const Shape> shapes) {
    ...
    for (const auto &shape : shapes | std::views::filter([](const auto &shape) {
                                 return queries::GetUnusualHeight(shape) > 50.0;
                             }) | std::views::take(3)) {
...
int main() {
    ...
    std::vector shapes_points(
        std::from_range,
        shapes | std::views::transform([](const auto &shape) {
