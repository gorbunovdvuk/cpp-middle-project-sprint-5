...
ShapeGenerator::ShapeGenerator(double min_coord, double max_coord, double min_size, double max_size)
...
std::vector<Shape> ShapeGenerator::GenerateShapes(size_t count) {
...
std::optional<size_t> FindUnusuallyHighestShape(const std::vector<Shape> &shapes) {
    ...
    return ranges::max_element(shapes, std::less<>{},
                               [](const auto &shape) { return queries::GetUnusualHeight(shape); }) -
           shapes.begin();
}
